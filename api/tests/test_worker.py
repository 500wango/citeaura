...
def test_celery_registers_all_pipeline_tasks():
    ...
    assert {
        ...
        "citeaura.reconcile_platform_usage",
    } <= registered
...
def test_reclaim_stale_jobs_releases_queued_job_lost_by_broker(tmp_path):
    engine = create_engine(f"sqlite:///{tmp_path / 'reclaim-queued.sqlite'}")
    Base.metadata.create_all(engine)
    session_factory = sessionmaker(bind=engine, autocommit=False, autoflush=False)
    now = datetime(2026, 8, 1, 12, 0, tzinfo=timezone.utc)
    with session_factory() as db:
        tenant = Tenant(name="reclaim-queued-tenant", plan="pro")
        db.add(tenant)
        db.flush()
        project = Project(
            tenant_id=tenant.id,
            slug="reclaim-queued",
            url="https://reclaim-queued.example",
            status="processing",
        )
        db.add(project)
        db.flush()
