...
UI_SUB.en.push(
  ['下载失败：','Download failed: '],['创建失败：','Creation failed: '],['保存失败：','Save failed: '],['更新失败：','Update failed: '],
  ['归档失败：','Archive failed: '],['恢复失败：','Restore failed: '],['同步失败：','Sync failed: '],['断开失败：','Disconnect failed: '],
  ['生成失败：','Generation failed: '],['发送失败：','Send failed: '],['邀请失败：','Invitation failed: '],['移除失败：','Removal failed: '],
  ['撤销失败：','Revocation failed: '],['切换失败：','Switch failed: '],['请填写 URL、更新诉求并选择至少一个影响问题','Enter a URL, requested update, and at least one affected prompt'],
  ['请输入完整恢复确认短语','Enter the complete restore confirmation phrase'],['请输入 Semrush API Key','Enter a Semrush API key'],
  ['确认断开 SMTP？','Disconnect SMTP?'],['请输入收件邮箱','Enter a recipient email'],['请勾选确认并输入完整确认短语','Select the confirmation checkbox and enter the complete confirmation phrase'],
  ['Logo 必须是 512KB 内的 PNG、JPEG 或 WebP','Logo must be a PNG, JPEG, or WebP file no larger than 512 KB'],
  ['请输入成员邮箱','Enter a member email'],['仅所有者可配置 API Key','Only owners can configure API keys'],['仅所有者可配置发布渠道','Only owners can configure publishing destinations']
);
UI_SUB.ja.push(
  ['下载失败：','ダウンロード失敗：'],['创建失败：','作成失敗：'],['保存失败：','保存失敗：'],['更新失败：','更新失敗：'],
  ['归档失败：','アーカイブ失敗：'],['恢复失败：','復元失敗：'],['同步失败：','同期失敗：'],['断开失败：','切断失敗：'],
  ['生成失败：','生成失敗：'],['发送失败：','送信失敗：'],['邀请失败：','招待失敗：'],['移除失败：','削除失敗：'],
  ['撤销失败：','取り消し失敗：'],['切换失败：','切替失敗：'],['请填写 URL、更新诉求并选择至少一个影响问题','URL、依頼する更新、影響するプロンプトを 1 件以上入力してください'],
  ['请输入完整恢复确认短语','完全な復元確認フレーズを入力してください'],['请输入 Semrush API Key','Semrush API キーを入力してください'],
  ['确认断开 SMTP？','SMTP 接続を解除しますか？'],['请输入收件邮箱','宛先メールを入力してください'],['请勾选确认并输入完整确认短语','確認チェックを選択し、完全な確認フレーズを入力してください'],
  ['Logo 必须是 512KB 内的 PNG、JPEG 或 WebP','ロゴは 512 KB 以下の PNG、JPEG、または WebP を使用してください'],
  ['请输入成员邮箱','メンバーのメールアドレスを入力してください'],['仅所有者可配置 API Key','API キーを設定できるのはオーナーのみです'],['仅所有者可配置发布渠道','公開先を設定できるのはオーナーのみです']
);
